...
%**************************************************************************
function out= myfun_Fsolve_Sol(input)
global param_myfun;
...
Ts=1-1./(1+exp(-sig_Alpha.*(x(2)-sig_sp)));
Tp=1./(1+exp(-sig_Alpha.*(x(2)-sig_sp))); 

F=[x(1) - ((-1/Delta_h)*(V_H * Tp - Betta_H * Ts - Delta_h * IHR));
   x(2) - ((1+Alpha*Ts) * R0_a * DeltaV * x(1))];
end
